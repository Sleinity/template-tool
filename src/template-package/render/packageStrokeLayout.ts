...
function resolveStrategy(
  ...
  if (mode === "static" || includedInLayout === null || !alignment) {
    return "border";
  }
  if (alignment === "INSIDE") {
    return includedInLayout ? "border" : "inset-shadow";
  }
...
export function resolvePackageStrokeModel(
  ...
  const includedInLayout =
    typeof rawIncludedInLayout === "boolean"
      ? rawIncludedInLayout
      : null;
  ...
  const strategy =
    paint && weight > 0
      ? resolveStrategy(mode, includedInLayout, alignment)
      : "none";
  const layers = solidStrokes
    .map((stroke): PackageStrokeLayer | null => {
      const layerPaint = strokePaint(
        stroke,
      ) as Extract<PackagePaint, { type: "SOLID" }>;
      const layerWeight = strokeWeight(
        stroke,
        node.appearance.strokeWeight ?? 1,
      );
      if (layerWeight <= 0) return null;
      const rawLayerAlignment =
        ("paint" in stroke ? stroke.align : null) ?? rawAlignment;
  ...
  return {
    ...
    alignment,
    rawAlignment:
      typeof rawAlignment === "string" ? rawAlignment.toUpperCase() : null,
    includedInLayout,
    rawIncludedInLayout,
    strategy,
    layers,
