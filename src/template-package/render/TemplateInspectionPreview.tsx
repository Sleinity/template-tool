...
import { Button, IconButton, TemplatePreviewStage } from "../../components/ui";
...
interface TemplateInspectionPreviewProps {
...
export function TemplateInspectionPreview({
  ...
  return (
    <div className={`template-inspection-preview ${className ?? ""}`} data-focus-mode={focusMode}>
      {showControls ? (
        <div className="inspection-preview-toolbar" aria-label="Preview controls">
          <Button
            ...
            aria-pressed={focusMode === "template"}
            onClick={() => fitTemplate()}
          >
            Fit template
          </Button>
          ...
          <div className="inspection-preview-toolbar__zoom">
            <IconButton label="Zoom out" icon={<ZoomOut size={16} />} onClick={() => zoom(0.8)} />
            <span className="min-w-12 text-center text-xs text-content-muted" aria-live="polite">
              {Math.round(transform.scale * 100)}%
            </span>
            <IconButton label="Zoom in" icon={<ZoomIn size={16} />} onClick={() => zoom(1.25)} />
          </div>
        </div>
      ) : null}
      <TemplatePreviewStage
        ...
        size="standard"
        className="inspection-preview-viewport"
