...
import type {
  BackendDiagnosticNodeInputV1,
  BackendWarningInputV1,
} from "./inputContracts";
...
export function createBackendDiagnosticProjection(
  nodes: Record<string, BackendDiagnosticNodeInputV1>,
  warnings: BackendWarningInputV1[],
  sourceDecisionRevision: string,
): ResolvedBackendDiagnosticProjectionV1 {
  const diagnostics: ResolvedBackendDiagnosticV1[] = [];
  const warningsByNode = new Map<string, BackendWarningInputV1[]>();
