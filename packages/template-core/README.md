...
```sh
npm install @sleinity/template-core@0.4.0
```
...
`sdk-v0.4.0` GitHub Release instead:

1. Verify `sleinity-template-core-0.4.0.tgz` against `SHA256SUMS`.
2. Commit it to the private Lovable-synced repository under `vendor/`.
3. Declare
   `"@sleinity/template-core":
   "file:vendor/sleinity-template-core-0.4.0.tgz"`.
...
locked to the SDK 0.4.0 contract at 87,431 bytes and SHA-256
